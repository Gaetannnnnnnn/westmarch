...
export function TmHooks() {
    ...
    Hooks.on("renderApplicationV2", (app, element) => {
        ...
        const tmFlag   = actor.getFlag("westmarch-ashara", "tm");
...
async function openDeclarationDialog(actor) {
    const existing = actor.getFlag("westmarch-ashara", "tm");
        ...
        render: () => {
            ...
            function refreshCart() {
                ...
                $html.find(".tm-remove-item").on("click", async function () {
                    ...
                    if (cartItems.length === 0) await actor.unsetFlag("westmarch-ashara", "tm");
                    else await actor.setFlag("westmarch-ashara", "tm", { declared: false, items: cartItems });
                    refreshCart();
                });
            }
            refreshCart();
            ...
            $html.find("#tm-add-to-cart").on("click", async () => {
                ...
                await actor.setFlag("westmarch-ashara", "tm", { declared: false, items: cartItems });
                refreshCart();
            });
        },
        buttons: [
            {
                ...
                callback: async (event, button, dialog) => {
                    ...
                    await actor.setFlag("westmarch-ashara", "tm", {
                        declared: true,
...
function buildActorRow(actor, startUnchecked = false) {
    const flag  = actor.getFlag("westmarch-ashara", "tm");
...
async function openDowntimeDialog() {
    ...
    const declared   = allActors.filter(a =>  a.getFlag("westmarch-ashara", "tm")?.declared);
    const undeclared = allActors.filter(a => !a.getFlag("westmarch-ashara", "tm")?.declared);
    ...
    await (foundry.applications.api.DialogV2 ?? DialogV2).wait({
        ...
        render: () => {
            ...
            $html.find(".tm-refuse-btn").on("click", async function () {
                ...
                await actor.unsetFlag("westmarch-ashara", "tm");
...
async function applyDowntimeGains($html, actors) {
    ...
    for (const actor of actors) {
        ...
        if (remainingItems.length > 0) {
            await actor.setFlag("westmarch-ashara", "tm", { declared: false, items: remainingItems });
        } else {
            await actor.unsetFlag("westmarch-ashara", "tm");
        }
    }
    ...
    const webhookUrl = game.settings.get("westmarch-ashara", "tmWebhookUrl");
    if (webhookUrl) {
        fetch(webhookUrl, {
            method: "POST",
...
function notifyTmDeclared() {
    ...
    const pending = game.actors.filter(a =>
        a.type === "character" &&
        a.hasPlayerOwner &&
        a.getFlag("westmarch-ashara", "tm")?.declared
    );
