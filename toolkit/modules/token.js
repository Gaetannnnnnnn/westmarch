...
export function TokenHooks() {
    ...
    if (game.modules.get("lib-wrapper")?.active) {
        try {
            libWrapper.register("toolkit", "CONFIG.Token.objectClass.prototype._onClickRight2", function (wrapped, ...args) {
    ...
    Hooks.on("renderTokenHUD", (hud, html, data) => {
        if (!game.settings.get("toolkit", "enableTokenAppearance")) return;
        ...
        const rawImages = actor.getFlag("toolkit", "images");
    ...
    // ============================================================
    Hooks.on("renderTokenHUD", (hud, html, data) => {
        if (!game.settings.get("toolkit", "enableTokenPortraitButton")) return;
        const token = hud.object;
        if (!token) return;
    ...
    Hooks.on("renderPrototypeTokenConfig", (app, html, data) => {
        ...
        section.find(".westmarch-add-image").click(() => {
            openImportPopup(async (entry) => {
                const current = actor.getFlag("toolkit", "images") ?? [];
                const updated = [...current, entry];
                await actor.setFlag("toolkit", "images", updated);
        ...
        section.on("click", ".westmarch-remove-image", async (ev) => {
            ...
            await actor.setFlag("toolkit", "images", [...current]);
