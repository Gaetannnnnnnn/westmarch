...
//
// Stockage :
//   page.flags["toolkit"]["restock"]        = { itemId: worldTimeExpiry }
//   page.flags["toolkit"]["restockEnabled"] = { itemId: true/false }
// ============================================================
...
export function MejRestockHooks() {
    ...
    Hooks.on("updateJournalEntryPage", async (page, change, options, userId) => {
        ...
        const enabled = page.flags?.["toolkit"]?.restockEnabled ?? {};
    ...
    Hooks.on("updateWorldTime", async (worldTime) => {
        ...
        for (const journal of game.journal.contents) {
            for (const page of journal.pages.contents) {
                ...
                const timers = page.flags?.["toolkit"]?.restock ?? {};
    ...
    Hooks.on("renderApplicationV2", (application, element) => {
        ...
        function injectRestockUI() {
            ...
            const currentTimers  = pg.getFlag("toolkit", "restock")        ?? {};
            const currentEnabled = pg.getFlag("toolkit", "restockEnabled")  ?? {};
            const spd = getSecondsPerDay();
            ...
            element.querySelectorAll(".items-list .item[data-id]").forEach(row => {
                ...
                btn.addEventListener("click", async (e) => {
                    ...
                    const nowEnabled = !!((freshPg.getFlag("toolkit", "restockEnabled") ?? {})[itemId]);
                    const newEnabled = foundry.utils.deepClone(freshPg.getFlag("toolkit", "restockEnabled") ?? {});
