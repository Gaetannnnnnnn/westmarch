...
function _injectTgcmButton(hud, html) {
    ...
    const isActive = tokenDoc.getFlag("toolkit", "tgcm") ?? false;
    ...
    btn.addEventListener("click", async (e) => {
        e.preventDefault();
        e.stopPropagation();
        const current = tokenDoc.getFlag("toolkit", "tgcm") ?? false;
        await tokenDoc.setFlag("toolkit", "tgcm", !current);
        hud.render();
    });
...
function _refreshTgcmShield(token) {
    ...
    if (!game.user.isGM) return;
    if (!token.document?.getFlag("toolkit", "tgcm")) return;
