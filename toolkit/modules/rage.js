...
async function applyGiantSize(actor) {
    ...
    for (const tokenDoc of actor.getActiveTokens(false, true)) {
        ...
        if (!tokenDoc.getFlag("toolkit", "preRageSize")) {
            await tokenDoc.setFlag("toolkit", "preRageSize", {
...
async function revertGiantSize(actor) {
    for (const tokenDoc of actor.getActiveTokens(false, true)) {
        const saved = tokenDoc.getFlag("toolkit", "preRageSize");
        if (!saved) continue;

        await tokenDoc.update({ width: saved.width, height: saved.height });
        await tokenDoc.unsetFlag("toolkit", "preRageSize");
    }
}
...
export function RageHooks() {
    ...
    Hooks.on("createActiveEffect", async (effect) => {
        if (!game.settings.get("toolkit", "enableRageSize")) return;
