...
async function applyTransform(tokenDoc, beastActor) {
    const alreadySaved = !!tokenDoc.getFlag("toolkit", "polymorphOriginal");
...
async function revertTransform(tokenDoc) {
    const saved = tokenDoc.getFlag("toolkit", "polymorphOriginal");
    if (!saved) return;
    ...
    await tokenDoc.unsetFlag("toolkit", "polymorphOriginal");
}
...
export function PolymorphHooks() {
    ...
    Hooks.on("renderTokenHUD", (hud, html, data) => {
        if (!game.settings.get("toolkit", "enablePolymorph")) return;
        ...
        const forms         = actor.getFlag("toolkit", "polymorphForms") ?? [];
        const isTransformed = !!token.document.getFlag("toolkit", "polymorphOriginal");
    ...
    Hooks.on("renderPrototypeTokenConfig", (app, html, data) => {
        ...
        const savedForms = actor.getFlag("toolkit", "polymorphForms") ?? [];
        ...
        section.find(".wm-poly-add-btn").on("click", async () => {
            ...
            const current = actor.getFlag("toolkit", "polymorphForms") ?? [];
            const updated = [...current, { actorId: pendingActorId, label }];
            await actor.setFlag("toolkit", "polymorphForms", updated);
            renderForms(updated);
        ...
        section.on("click", ".wm-poly-remove", async (ev) => {
            ...
            await actor.setFlag("toolkit", "polymorphForms", [...current]);
            renderForms(current);
        });
