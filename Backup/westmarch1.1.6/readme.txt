...
v1.1.6 | 2026-06-22
  - Ajout de settings.js
    → Toutes les fonctionnalités sont désormais activables/désactivables par les GM
    → Accessible via Paramètres du jeu → Configuration des modules → WestMarch Système
    → Chaque paramètre dispose d'une description au survol de la souris

  - Ajout du setting maître "Système de Party" (enableParty)
    → Conditionne désormais Create/Join/Leave/Kick/Invite Party (auparavant
      non protégés par aucun setting)
    → 7 sous-options en dépendent en cascade fonctionnelle : Join Scene, Show
      Party, Filtrage du chat, Regroupement visuel, Go With Party (scènes et
      journaux), Journal de session — désactivées même si cochées si le
      Système de Party est désactivé
    → Dans le menu de configuration, ces 7 sous-options sont désormais
      indentées et reliées visuellement sous "Système de Party", qui affiche
      un symbole ⚠️ avec une infobulle listant les options concernées

  - Ajout de anticheat.js (Anti-Cheat en combat)
    → Surveille, pendant un combat actif, les changements sur les sorts
      préparés, l'attunement et l'équipement (armes/armures) des personnages
      joueurs engagés dans le combat
    → Envoie un message d'avertissement dans le chat visible uniquement par
      les GM (whisper), avec le nom du personnage, du joueur et de l'objet/sort
    → Les modifications des GM ne déclenchent jamais d'alerte
    → Nouveau setting "Anti-Cheat (combat)" (enableAntiCheat)

  - Ajout de session.js (Journal de session)
    → Bouton "Clore la session" sous la liste des joueurs (GM uniquement)
    → Nouvelle option clic droit "Create Party with Log" (player.js) qui démarre
      la capture en plus de créer la party
    → Capture automatique : XP avant/après + level up, ennemis du combat tracker
      (CR, HP, CA, actions/résistances légendaires), PNJ sur la scène, objets
      ajoutés aux inventaires
    → Génère un journal dans MJ/<nom du GM>/Rapport de session/<date> à la
      clôture, puis dissout la party
    → Nouveau setting "Journal de session" (enableSessionLog)

  - Ajout de token.js
    → Les GM peuvent configurer plusieurs images sur un token (config du token → onglet Apparence)
    → Les joueurs peuvent cycler entre les images via un bouton ▶ dans le HUD du token

  - Activation de la fonction "Join Scene"
    → Désormais activable via les paramètres du module sans modifier le code
    → Permet aux joueurs de rejoindre la scène d'un membre de leur party
    → Ne permet pas de rejoindre la scène d'un GM