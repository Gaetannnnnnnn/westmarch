    ...
    Hooks.on("renderPlayers", (app, html, data) => {
        ...
        const closeBtn = $(`
            <div style="padding: 4px 8px; margin-top: 4px;">
                <button type="button" class="westmarch-close-session" style="width:100%; font-size:12px;">
                    <i class="fas fa-book"></i> Clore la session
                </button>
            </div>
        `);
        closeBtn.find('button').click(() => closeSession(app));
...
async function createSessionJournal(date, content) {
    // Trouver le dossier racine "MJ"
    let rootFolder = game.folders.find(f => f.name === "MJ" && f.type === "JournalEntry" && !f.folder);
    if (!rootFolder) {
        ui.notifications.error("Le dossier 'MJ' est introuvable.");
        return;
    }
